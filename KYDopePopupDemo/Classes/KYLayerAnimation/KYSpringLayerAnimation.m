...
-(CAKeyframeAnimation *)createCustomTransformAnima:(NSString *)keypath duration:(CFTimeInterval)duration fromValue:(id)fromValue toValue:(id)toValue{
    ...
    for (NSInteger frame = 0; frame<numOfFrames; frame++) {
        ...
        CGFloat value_for_rotate = [fromValue floatValue] + diff * (-4* x *(x-1));
        CGFloat value_for_scale = [fromValue floatValue] + diff * (-x *(x-2));
        
//        CGFloat value_for_rotate = x <=0.2? 0:[fromValue floatValue] + diff * (-1/0.16*(x-0.2)*(x-1));
